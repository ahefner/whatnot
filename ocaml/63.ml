...
number of nodes, all the nodes are "left-adjusted". This means that in
a levelorder tree traversal all internal nodes come first, the leaves
...
following specification: is_complete_binary_tree n t returns true iff
t is a complete binary tree with n nodes.  *)

let succ_left index = 2 * index
let succ_right index = 2 * index + 1

let complete_binary_tree elements =
  let array = Array.of_list elements in
  let rec build index =
    if index > (Array.length array)
    then Empty
    else Node (Array.get array (index-1),
